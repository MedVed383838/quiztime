...
describe("quiz management", () => {
  ...
  it("creates, updates and deletes questions with strict option validation", async () => {
    const created = await agent.post(`/api/quizzes/${quizId}/questions`).send({
      text: "Сколько будет два плюс два?", type: "SINGLE_CHOICE", timeLimitSeconds: 45, options: [
        { text: "4", isCorrect: true }, { text: "5", isCorrect: false },
      ],
    });
    expect(created.status).toBe(201);
    expect(created.body.data.question.options).toHaveLength(2);
    expect(created.body.data.question.timeLimitSeconds).toBe(45);
    ...
    const updated = await agent.put(`/api/questions/${questionId}`).send({ text: "Обновлённый вопрос", type: "MULTIPLE_CHOICE", timeLimitSeconds: 15, options: [{ text: "4", isCorrect: true }, { text: "Четыре", isCorrect: true }, { text: "5", isCorrect: false }] });
    expect(updated.status).toBe(200);
    expect(updated.body.data.question.type).toBe("MULTIPLE_CHOICE");
    expect(updated.body.data.question.timeLimitSeconds).toBe(15);
